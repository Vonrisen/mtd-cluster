...
Add Grafana Helm Repository: Add the official Grafana Helm chart repository:

```bash
helm repo add grafana [https://grafana.github.io/helm-charts](https://grafana.github.io/helm-charts)
Update Helm Repositories: Fetch the latest charts from all added repositories:
```
```bash
helm repo update
Install Grafana using Helm: Install Grafana into a dedicated namespace called monitoring. We will expose the Grafana UI using a NodePort service for easy access in a demo environment.
```
...
Find the NodePort assigned to the Grafana service:

```bash
kubectl get services -n monitoring grafana
```
Look for the NODEPORT listed in the output (e.g., 30880:30000/TCP).
Open your web browser and navigate to http://<Master_Static_IP>:<Grafana_NodePort>. Replace <Master_Static_IP> with the actual static IP address of your Master node and <Grafana_NodePort> with the NodePort you found in the previous step.
Login using the default credentials: Username admin, Password admin (or the password you set with --set adminPassword). Change this password immediately after logging in!
Configure Prometheus Data Source in Grafana: KubeSphere typically includes a Prometheus instance for cluster monitoring. You need to configure Grafana to use this Prometheus as a data source to visualize metrics.

Once logged into the Grafana dashboard, click "Connections" (or the plug icon) in the left-hand side panel, then select "Data Sources".
Click "Add data source" and choose "Prometheus" from the list.
In the "Settings" panel, find the "Connection" section.
For the "URL", you need the in-cluster service address of your Prometheus instance. If you are using the default Prometheus installed by KubeSphere, it's typically named prometheus-k8s and resides in the kubesphere-monitoring-system or monitoring namespace, exposed on port 9090. The typical in-cluster URL format is http://<service-name>.<namespace>.svc:<port>.
You can verify the service name and namespace using:
Bash

kubectl get services -A | grep prometheus
A common URL for KubeSphere's Prometheus would be:
[http://prometheus-k8s.kubesphere-monitoring-system.svc:9090](http://prometheus-k8s.kubesphere-monitoring-system.svc:9090)
Enter the correct Prometheus service URL in the URL field.
Scroll down and click the "Save & test" button.
You should see a green confirmation box stating "Successfully queried the Prometheus API."
...
Install MySQL Exporter: Install the Prometheus MySQL Exporter using Helm to collect metrics from your MySQL database.
...
helm repo update
Install the MySQL Exporter. You will need a values file (<path_to_your_values_file>) containing the connection details for your MySQL database.
Bash
...
Upon successful installation, you will see output similar to the one you provided:
NAME: mysql-exporter
...
1. Get the application URL by running these commands:
   ...
   echo "Visit [http://127.0.0.1:9104](http://127.0.0.1:9104) to use your application"
...
Configure Frontend and Backend Metrics: Your frontend and backend services need to expose metrics in a format Prometheus understands (usually /metrics endpoint in the Prometheus text format). If they already do this, you need to tell Prometheus how to find them, often using a ServiceMonitor resource if you are using the Prometheus Operator (which is common in KubeSphere).

Apply ServiceMonitors: Apply the YAML files that define how Prometheus should scrape metrics from your frontend and backend services.
Bash
...
Allow a couple of minutes for Prometheus to discover these new ServiceMonitor targets and start scraping metrics.
Import Dashboards into Grafana: Import pre-built Grafana dashboards designed to visualize the metrics collected from MySQL, Frontend, and Backend services.

Go to the Grafana dashboard in your browser.
Click "Dashboards" (or the four squares icon) in the left-hand side panel, then select "New" and "Import".
You will typically be prompted to upload a .json file or paste JSON text for the dashboard.
Import the JSON files for your mysql_dashboard.json, frontend_dashboard.json, and backend_dashboard.json one by one. During the import process, ensure you select the Prometheus data source you configured earlier when prompted.
...
helm repo update
Install Falco: Install Falco into a dedicated namespace. The --replace flag can be used if you need to reinstall, but be cautious as it will delete and recreate resources.
...
Wait for Falco Pods: Wait for the Falco pods to start and become Running. You can monitor their status:
...
Install Falco Sidekick: Falco Sidekick is a companion service that routes Falco alerts to various outputs. We will configure it to expose metrics that Prometheus can scrape.
...
Configure Prometheus to Scrape Falco Sidekick Metrics: Apply a ServiceMonitor resource to tell Prometheus how to scrape the metrics exposed by Falco Sidekick.
